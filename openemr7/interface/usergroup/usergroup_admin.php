...
if (isset($_POST["privatemode"]) && $_POST["privatemode"] == "user_admin") {
    if ($_POST["mode"] == "update") {
        ...
        //    $fd= implode(',',$_POST["facility_id"]);
        //    print_r($fd);
        //    die();
            sqlStatement("update users set facility_id = ? where id = ? ", array(implode(',',$_POST["facility_id"]), $_POST["id"]));
            //(CHEMED) Update facility name when changing the id
            sqlStatement("UPDATE users, facility SET users.facility = facility.name WHERE facility.id = ? AND users.id = ?", array(implode(',',$_POST["facility_id"]), $_POST["id"]));
            // END (CHEMED)
        }
...
if (isset($_POST["mode"])) {
    if ($_POST["mode"] == "new_user") {
        ...
        if ($doit == true) {
            ...
            "', facility_id = '"   . add_escape_custom(trim((isset($_POST['facility_id']) ? implode(',',$_POST['facility_id']) : ''))) .
            ...
            if ($success) {
                ...
                sqlStatement(
                    ...
                    array(
                        $uuid,
                        trim((isset($_POST['facility_id']) ? implode(',',$_POST['facility_id']) : '')),
                        trim((isset($_POST['rumple']) ? $_POST['rumple'] : ''))
                    )
                );
