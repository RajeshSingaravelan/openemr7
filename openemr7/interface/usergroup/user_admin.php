...
<td><span class=text><?php echo xlt('Default Facility'); ?>: </span></td><td><select name="facility_id[]"  multiple="multiple"  style="width:150px;" class=" form-control">
<?php
...
if ($fres) {
    ...
    foreach ($result as $iter2) {
        ...
        $facility_id = explode(",",$iter['facility_id']);
        // if ($iter['facility_id'] == $iter2['id'])
      
        if (in_array($iter2['id'], array_map('trim', $facility_id)) )
        ...
        ?>
          ...
          <option value="<?php echo attr($iter2['id']); ?>" <?php if ($iter['facility_id'] == $iter2['id']) {
                echo "selected";
                         } ?> <?php echo  $str_flag; ?>><?php echo text($iter2['name']); ?></option>
        <?php
    }
}
?>
